    ...
    <svg height={daysLinesHeight + gapBottom} width={hoursLinesWidth + gapRight}>
      ...
      {#each Object.entries($plan) as [day, slots], index (day)}
        {#each slots as slot, i}
          <rect
            x={dayToXCoordinate(day)}
            y={timeToYCoordinate(slot.start)}
            rx="10"
            ry="10"
            width={daysWidth}
            height={durationToLength(slot.duration)}
            style="fill:rgb(100,116,139);stroke-width:3;stroke:rgb(255,255,255)"
          />
        {/each}
      {/each}
    </svg>
