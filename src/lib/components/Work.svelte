...
<div class="p-4 w-72 shadow-xl bg-orange-200 border-solid border-2" data-popup={edit_targer_popup}>
  ...
  <label class="label">
    <span>Name</span>
    <input bind:value={work_name_edit} class="input rounded pl-1" type="text" />
  </label>

  <label class="label">
    <span>Description</span>
    <input bind:value={work_description_edit} class="input rounded pl-1" type="text" />
  </label>

  <label class="label">
    <span>Actual Duration</span>
    <div class="flex">
      <span class="flex-initial w-16"><input bind:value={work_actual_duration_hours_edit} class="input rounded pl-1" type="number" min="0" step="1" /></span>
      <span class="flex-initial w-8 text-lesft pl-1 text-lg">h :</span>
      <span class="flex-initial w-16"><input bind:value={work_actual_duration_minutes_edit} class="input rounded pl-1" type="number" min="0" max="59" step="1" /></span>
      <span class="flex-initial w-8 text-left pl-1 text-lg">m</span>
      <span class="flex-initial w-16"><input bind:value={work_actual_duration_seconds_edit} class="input rounded pl-1" type="number" min="0" max="59" step="1" /></span>
  ...
  <label class="label">
    ...
    <div class="flex">
      <span class="flex-initial w-16"><input bind:value={work_expected_duration_hours_edit} class="input rounded pl-1" type="number" min="0" step="1" /></span>
      <span class="flex-initial w-8 text-lesft pl-1 text-lg">h :</span>
      <span class="flex-initial w-16"><input bind:value={work_expected_duration_minutes_edit} class="input rounded pl-1" type="number" min="0" max="59" step="1" /></span>
      <span class="flex-initial w-8 text-left pl-1 text-lg">m</span>
    </div>
  </label>
  ...
  <button
    type="button"
    class="btn btn-sm mt-2 variant-filled bg-amber-800"
    on:click={() => {
      resetEdit();
    }}
  >
  ...
  <button
    type="button"
    class="btn btn-sm mt-2 ml-1 variant-filled bg-green-500"
    on:click={() => {
      saveWork();
    }}
  >
